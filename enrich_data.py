...
import collections
default_data = {"type": "private", "label": "Private / Independent", "est": "2010", "founder": "Founding Members", "head": "Director / Head"}
...
css = """
        ...
        .tt-type.govt { background: #fee2e2; color: #b91c1c; }
        .tt-type.semi { background: #fef3c7; color: #b45309; }
        .tt-type.private { background: #e0e7ff; color: #4338ca; }
...
# INJECT HTML IN CARDS
def replacer(match):
    full_card = match.group(0)
    
    # Extract the name
    name_match = re.search(r'<a href="[^"]+" class="tt-name">([^<]+)</a>', full_card)
    if not name_match:
        name_match = re.search(r'<h[34][^>]*class="tt-name"[^>]*>([^<]+)</', full_card)
        if not name_match:
            name_match = re.search(r'class="tt-name"[^>]*>([^<]+)', full_card)
            if not name_match:
                return full_card
    ...
    if 'tt-type' in full_card and 'tt-details' in full_card:
        return full_card

    # Get data
    info = data.get(name, default_data)
    
    # Create the block to insert
    insert_block = f'''<div class="tt-type {info['type']}">{info['label']}</div>\n<div class="tt-header">'''
    
    # We replace <div class="tt-header"> with the new blocks
    full_card = re.sub(r'<div class="tt-header">', insert_block, full_card, count=1)
    
    details_block = f'''</div><div class="tt-details">
    <div class="tt-detail-item"><span class="tt-detail-label">Est. Year</span><span class="tt-detail-value">{info['est']}</span></div>
    <div class="tt-detail-item"><span class="tt-detail-label">Founder</span><span class="tt-detail-value">{info['founder']}</span></div>
    <div class="tt-detail-item"><span class="tt-detail-label">Head</span><span class="tt-detail-value">{info['head']}</span></div>
</div><div class="tt-meta">'''
    
    full_card = re.sub(r'</div><div class="tt-meta">', details_block, full_card, count=1)
    
    return full_card

# Regex to find <div class="tt-card ..."> ... </div>
html = re.sub(r'<div class="tt-card [^>]+>.*?</div></div></div>', replacer, html, flags=re.DOTALL)
html = re.sub(r'<div class="tt-card [^>]+>.*?(?=<(?:div class="tt-card|/div></div></div>|<div class="domain-row|<div class="filter-bar))', replacer, html, flags=re.DOTALL)

# A more robust card finder
def robust_replacer():
    with open('/Users/roshanamarujala/Desktop/Antigravity/index.html', 'r', encoding='utf-8') as fx:
        html2 = fx.read()
    ...
    if '.tt-type {' not in style_tag.string:
        style_tag.string = style_tag.string + css
    ...
    for card in cards:
        ...
        # Insert TT-Type after tt-social or before tt-header
        type_div = soup.new_tag("div", attrs={"class": f"tt-type {info['type']}"})
        type_div.string = info['label']
        
        header = card.find('div', class_='tt-header')
        if header:
            header.insert_before(type_div)
            
        # Create tt-details
        details_div = soup.new_tag("div", attrs={"class": "tt-details"})
        
        # Est year
        item1 = soup.new_tag("div", attrs={"class": "tt-detail-item"})
        lbl1 = soup.new_tag("span", attrs={"class": "tt-detail-label"})
        lbl1.string = "Est. Year"
        val1 = soup.new_tag("span", attrs={"class": "tt-detail-value"})
        val1.string = info['est']
        item1.append(lbl1)
        item1.append(val1)
        
        # Founder
        item2 = soup.new_tag("div", attrs={"class": "tt-detail-item"})
        lbl2 = soup.new_tag("span", attrs={"class": "tt-detail-label"})
        lbl2.string = "Founder"
        val2 = soup.new_tag("span", attrs={"class": "tt-detail-value"})
        val2.string = info['founder']
        item2.append(lbl2)
        item2.append(val2)
        
        # Head
        item3 = soup.new_tag("div", attrs={"class": "tt-detail-item"})
        lbl3 = soup.new_tag("span", attrs={"class": "tt-detail-label"})
        lbl3.string = "Head"
        val3 = soup.new_tag("span", attrs={"class": "tt-detail-value"})
        val3.string = info['head']
        item3.append(lbl3)
        item3.append(val3)
        
        details_div.append(item1)
        details_div.append(item2)
        details_div.append(item3)
        
        # Insert before tt-meta
        meta = card.find('div', class_='tt-meta')
        if meta:
            meta.insert_before(details_div)
            
    with open('/Users/roshanamarujala/Desktop/Antigravity/index.html', 'w', encoding='utf-8') as fx:
        fx.write(str(soup))
